...
from environment2 import GridWorldEnv
...
def main():
    ...
    def test_env():
        ...
        for ii in range(num_steps):
            ...
            state = torch.tensor(state).unsqueeze(0).to(torch.float)
            ...
            state, reward, _, done = env.step(action.item())
    ...
    # we reset the env
    # state is (x, y, pick_axe, has_gem, at_door, in_lava)
    state, reward, comments, done = env.reset()
    ...
    train_acc_rews_mean_arr = []

    mean_train_reward = 0

    counter_train_reward = 0
    ...
    while frame_idx < max_frames:
        ...
        for ii in range(num_steps):
            ...
            episode_train_acc_reward+=float(reward)*10

            # storing the reward retrived from the env
        ...
        #mean_train_reward = (mean_train_reward + episode_train_acc_reward) / (frame_idx+1)



        train_acc_rews.append(episode_train_acc_reward)

        train_acc_rews_mean_arr.append(np.mean(train_acc_rews[:50]))

        plt.plot([i for i in range(len(train_acc_rews_mean_arr))], train_acc_rews_mean_arr)
        plt.xlabel("episode")
        plt.ylabel("mean of train acc rew")
        plt.savefig("train_acc_rewards"+".png")
        plt.close()
        ...
        if frame_idx % TT == 0:
            ...
            if(frame_idx != 0):
                ...
                xaxe.append(counter_for_loss)

                #plt.plot(mean_losses, xaxe)
                plt.plot(xaxe, mean_losses)
                plt.xlabel("episode")
        ...
        if frame_idx % TTT == 0:
            ...
            plt.tight_layout()
            plt.savefig("rewardsVSepisode"+".png")
            plt.close()
