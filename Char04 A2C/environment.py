...
class GridWorldEnv(gym.Env):
    ...
    def reset(self):
        ...
        info = self._get_info()
        reward = -3
        return observation, reward, info, False
    ...
    def step(self, action):
        ...
        observation = self._get_obs()
        info = self._get_info()
        return observation, reward, info, termination
...
#         self._pickaxe_location = np.array([3, 2])
#         self._exit_location = np.array([3, 0])
#         self._lava_location = np.array([2, 0])
...
#         if self._check_lava():
#             return True
#         elif (self._agent_location == self._exit_location).all() and self._has_gem:
#             return True
#         else:
#             return False
...
#         has_pick = self._check_pickaxe()
#         if self._has_pickaxe:
#             reward = 2
#         has_gem = self._check_gem()
#         if self._has_gem:
#             reward = 1
...
class GridWorldEnvND(gym.Env):
    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 4}
    
    def __init__(self, dfa, render_mode = "human", train = True, size = 4, p=0.8):
        ...
        self.probability_correct_action = p
        self.size = size #4x4 world
        self.window_size = 512 #size of the window
    ...
    def step(self, action):
        ...
        if random.random() <= 1 - self.probability_correct_action:
            if random.random() < 0.5:
                self._agent_location = np.clip(self._agent_location+self._action_to_direction[(action+1)%4], 0, self.size-1)
            else:
                self._agent_location = np.clip(self._agent_location+self._action_to_direction[(action-1)%4], 0, self.size-1)
